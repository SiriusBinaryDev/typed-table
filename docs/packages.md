...
- `examples/basic-table` covers local sorting, filtering, manual row ordering, grouping, expandable grouped rows, pagination, row selection, column visibility, headless column sizing controls, shift-click multi-sort, and local faceted filter metadata.
