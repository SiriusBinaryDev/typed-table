...
type RemoteSnapshot<TData> = {
  ...
  rowExpansion: TableState["rowExpansion"];
};
...
function createGroupingDependencyKey<TData>(grouping: GroupingState<TData>): string {
  return JSON.stringify(grouping ?? []);
}

function createRowExpansionDependencyKey(rowExpansion: TableState["rowExpansion"]): string {
  return JSON.stringify(
    Object.entries(rowExpansion).sort(([leftKey], [rightKey]) =>
      leftKey.localeCompare(rightKey),
    ),
  );
}
...
function shouldAppendRemoteSnapshot<TData>(
  ...
  return (
    ...
    areStringArraysEqual(previous.grouping, next.grouping) &&
    areFiltersEqual(previous.rowExpansion, next.rowExpansion)
  );
}
...
export function useTable<TData>(config: UseTableConfig<TData>): TableInstance<TData> {
  ...
  const groupingDependencyKey = createGroupingDependencyKey(state.grouping as GroupingState<TData>);
  const rowExpansionDependencyKey = createRowExpansionDependencyKey(state.rowExpansion);
  ...
  useEffect(() => {
    ...
    config.columns,
    filteringEnabled,
    groupingDependencyKey,
    groupingEnabled,
    isRemote,
    paginationEnabled,
    remoteLoadingMode,
    rowExpansionDependencyKey,
    rowExpansionEnabled,
    rowSelectionEnabled,
    sortingEnabled,
  ...
  function mapRemoteRow(row: TableRow<TData>): TableRow<TData> {
    const cellsByColumnId = new Map(row.cells.map((cell) => [cell.columnId, cell]));

    return {
      ...row,
      isSelected: row.type === "group"
        ? false
        : advancedRemoteRowSelectionEnabled
          ? isRemoteRowSelected(remoteAdvancedRowSelection, row.id)
          : row.isSelected,
      subRows: row.subRows.map((subRow) => mapRemoteRow(subRow)),
  ...
  const remoteRows = remoteModel.rows.map((row) => mapRemoteRow(row));
  ...
  const model: TableModel<TData> = isRemote
    ? {
        ...
        facetedUniqueValues: remoteModel.facetedUniqueValues,
        facetedMinMaxValues: remoteModel.facetedMinMaxValues,
      }
  ...
  function getFacetedUniqueValuesValue(
    ...
    if (isRemote || !localData) {
      return model.facetedUniqueValues?.[columnId] ?? [];
    }
  ...
  function getFacetedMinMaxValuesValue(
    ...
    if (isRemote || !localData) {
      return model.facetedMinMaxValues?.[columnId] ?? null;
    }
