...
function normalizeRowOrder(rowOrder: readonly string[]): RowOrderState {
  const seenRowIds = new Set<string>();
  const normalizedRowOrder: string[] = [];
  ...
  for (const rowId of rowOrder) {
    ...
    normalizedRowOrder.push(rowId);
  }

  return normalizedRowOrder;
}
...
export function setRowOrder(
  ...
  return {
    ...state,
    rowOrder: normalizeRowOrder(rowOrder),
  };
}
...
export function moveRow(
  ...
  const currentOrder = getOrderedColumnIds(availableRowIds, state.rowOrder);

  if (!currentOrder.includes(rowId)) {
    return state;
  }

  const nextOrder = currentOrder.filter((entry) => entry !== rowId);
  const normalizedTargetIndex = Math.min(
    Math.max(Math.floor(targetIndex), 0),
    nextOrder.length,
  );

  nextOrder.splice(normalizedTargetIndex, 0, rowId);

  return setRowOrder(state, nextOrder);
}

export function clearRowOrder(state: TableState): TableState {
  return {
    ...state,
    rowOrder: [],
  };
