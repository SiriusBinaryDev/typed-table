...
function orderUsersByRowOrder(rowOrder: readonly string[]): User[] {
  const rowOrderIndex = new Map(rowOrder.map((rowId, index) => [rowId, index]));

  return [...users].sort((left, right) => {
    const leftIndex = rowOrderIndex.get(left.id);
    const rightIndex = rowOrderIndex.get(right.id);
...
function UsersTable() {
  ...
  const activeGrouping = table.grouping.length
    ...
    : "None";
  const orderedUsers = orderUsersByRowOrder(table.rowOrder);
  const orderedUserIds = orderedUsers.map((user) => user.id);

  function handleRowOrderDrop(overId: string): void {
    if (!draggingRowId) {
      return;
    }

    table.setRowOrder(reorderRowIds(orderedUserIds, draggingRowId, overId));
  ...
  return (
    <div className="basic-stack">
      <section className="basic-panel basic-panel-soft basic-controls">
        ...
        <div className="basic-column-controls">
          <div className="basic-inline-header">
            <span className="basic-field-label">Row order</span>
            <button
              className="basic-text-button"
              disabled={table.rowOrder.length === 0}
              onClick={table.clearRowOrder}
              type="button"
            >
              Reset order
            </button>
          ...
          <div className="basic-row-order-list">
            {orderedUsers.map((user, index) => (
              <div
                key={user.id}
                className={[
                  "basic-row-order-card",
                  draggingRowId === user.id ? "is-dragging" : "",
                  dropRowId === user.id && draggingRowId !== user.id ? "is-drop-target" : "",
                ].filter(Boolean).join(" ")}
                onDragOver={(event) => {
                  event.preventDefault();
                  if (draggingRowId && draggingRowId !== user.id) {
                    setDropRowId(user.id);
                  }
                }}
                onDrop={(event) => {
                  event.preventDefault();
                  handleRowOrderDrop(user.id);
                }}
              >
                <div className="basic-row-order-leading">
                  <button
                    aria-label={`Drag to reorder ${user.name}`}
                    className="basic-drag-handle"
                    draggable
                    onDragEnd={() => {
                      setDraggingRowId(null);
                      setDropRowId(null);
                  ...
                  <div className="basic-row-order-summary">
                    <strong>{user.name}</strong>
                    <span className="basic-chip-meta">{user.team}</span>
                  </div>
                </div>
                <div className="basic-inline-actions">
                  <button
                    className="basic-size-button"
                    disabled={index === 0}
                    onClick={() => table.moveRow(user.id, index - 1)}
                    type="button"
                  >
                    Up
                  </button>
          ...
          <p className="basic-helper">
            Drag with the handle or use Up/Down. This reorder surface uses the exported
            <code> reorderRowIds(...)</code> helper, so the table still owns the actual
            <code> rowOrder</code> state. Active sorting or grouping can still change the visible
            table order after that base order is applied.
          </p>
        </div>
    ...
    <main className="basic-example">
      <section className="basic-hero">
        ...
        <p className="basic-lede">
          A compact people table with filtering, sorting, drag-and-drop row ordering, grouping,
          expandable grouped rows, pagination, row selection, column visibility, and host-rendered
          column sizing. Shift-click headers to add secondary sorts.
        </p>
      </section>
